...
class ActivateAccount extends React.Component {
  ...
  handleSubmit(e) {
    ...
    axios
      .post(`${process.env.API_URL}/auth/account-activation`, {
        token,
      })
      .then((response) => {
  ...
  render() {
    ...
    const title = visible ? (
      <h1>Hey, {username}, Are you ready to activate your account?</h1>
    ) : (
      <h1>Your account was successfully activated! You can now login!</h1>
    );
    return (
      <>
        {title}
        <button disabled={!visible} onClick={(e) => this.handleSubmit(e)}>
          {buttonText}
        </button>
      </>
    );
